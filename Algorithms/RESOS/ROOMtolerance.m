function [fluxROOM, solutionROOM, totalFluxDiff,MILPproblem] = ROOMtolerance(model, fluxWT, rxnKO, varargin)
%relaxes a feasibility  if a model is too constraint in gurobi
...
%solutionROOM = solveCobraMILP(MILPproblem, 'printLevel', printLevel);
        ...
        changeCobraSolverParams('MILP', 'feasTol', currentParameter);
        ...
        %params.NumericFocus is added and params.FeasibilityTol is increasing until a solution is found;
         while flag == 0
                ...
                changeCobraSolverParams('MILP', 'feasTol', currentParameter);
                if currentParameter == 1e-3
                    flag = 1;
                    continue
                end
         end
         
         changeCobraSolverParams('MILP', 'feasTol', originalParameter);
