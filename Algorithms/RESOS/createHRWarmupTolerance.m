function warmupPts = createHRWarmupTolerance(model, nPoints, verbFlag, bias, nPointsCheck)
%%%%%%%%%%%%%%%%%%%%Modified By Fernando Silva-Lance 2021 %%%%%%%%%%%%%%%%%
 %For highly constraint models, it changes feasibility tolerance parameter
 %to try to have a result, if the tolerance reaches a certain limit it
 %considers the model infeasible.
 %If you want to see the modifications done look for the word: MODIFICATION
...
while i <= nPoints/2
    ...
    for maxMin = [1, -1]
        ...
        %MODIFICATION
        flag = 0;
        originalParameter = 1e-6 ;
        currentParameter = originalParameter;
        %getCobraSolverParams('LP','feasTol');
        changeCobraSolverParams('LP', 'feasTol', currentParameter);
        while flag == 0
                sol = solveCobraLP(LPproblem);
                if sol.stat == 1
                    flag =1;
                ...
                currentParameter = currentParameter * 10;
                changeCobraSolverParams('LP', 'feasTol', currentParameter);
                if currentParameter == 1e-2
                    flag = 1;
                    continue
                end
        end
         changeCobraSolverParams('LP', 'feasTol', originalParameter);
         %END OF MODIFICATION
...
%achicar los warmup points para que sean aquellos que se encuentran dentro
%de la distancia aceptada por los warmup points de donde se sacó el
%percentil
