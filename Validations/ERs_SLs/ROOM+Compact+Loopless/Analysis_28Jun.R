...
#
##Load data
LethalRxns_Pipe<- read_delim("Desktop/ERs_Compacted_Loopless_28jun.csv", 
                                           delim = ";", escape_double = FALSE, col_names = FALSE, 
                                           trim_ws = TRUE)

LethalRxns_Exp_7Jun <- read_csv("LethalRxns_Exp_7Jun.csv") #"Experimental" lethal reaction - from lethal genes
...
113<- LethalRxns_Pipe_Final %>% drop_na() #:) s

LethalRxns_Pipe<-unique(LethalRxns_Pipe_Final) #Predicted from pipeline w/ compact model - check for duplicates
...
#Order or the contingency table / matrix -> (TP/FP/FN/TN)
  ...
  #~~~~~~~~~~~~~~
  # Load data ##
Synthetic_lethalRxns_Exp <- read_csv("Desktop/ERs_SLs/ExperimentalData/Synthetic lethalRxns_Exp_7Jun.csv")
Doubles1 <- read_delim("Desktop/SL1_CompactedLoopless_28Jun.csv", delim = ";", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
Doubles2 <- read_delim("Desktop/SL2_CompactedLoopless_28Jun.csv", delim = ";", escape_double = FALSE, col_names = FALSE,  trim_ws = TRUE) 
#
...
#
## load data 
LethalRxns_SL_9Jun <- LethalRxns_SL_9Jun <- read_csv("SingleLethal_Original+FASTSL.csv", col_names = FALSE)#Predicted lethal reactions  - normal model 

  LethalRxns_SL <- read_csv("Desktop/ERs_Original_28Jun.csv", 
                                 col_names = FALSE)
  colnames(LethalRxns_SL)<-'RXNS'
## Values for contingency matrix  ##
TPNormlist<- LethalRxns_SL %>% filter(LethalRxns_SL$RXNS %in% LethalRxns_Exp$RXNS)  #94 <-true positive
  TPNormlist<-94
  FPNorm<- LethalRxns_SL %>% filter(LethalRxns_SL$RXNS %!in% LethalRxns_Exp$RXNS)  #175 <- false positive
TNNormlist<- AllRxns %>% filter(AllRxns$X1 %!in% LethalRxns_SL$RXNS) 
TNNormlist <- TNNormlist %>% filter(TNNormlist$X1 %!in%  LethalRxns_Exp$RXNS) #2147 <- true negative
  TNNorm<- 2147
...
################################## SYNTHETIC LETHALS ############################
SL_normal <- read_delim("Desktop/SL_Original+FASTSL.csv", delim = ";", escape_double = FALSE, trim_ws = TRUE)

SL_normal<- read_delim("Desktop/SLs_Original_28Jun.csv", delim = ";", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
...
#
##Values for contingency matrix 
TPSLlist<- SL_normal %>% filter((SL_normal$Var1 %in% Gene_CorrectedSynthetic_lethal$X1 & SL_normal$Var2 %in% Gene_CorrectedSynthetic_lethal$X2) |
                                    (SL_normal$Var1 %in% Gene_correctedLethalRxns$Rxns & SL_normal$Var2 %in% Gene_correctedLethalRxns$Rxns)) 
TP<- 28 

FPSLlist<- SL_normal %>% filter((SL_normal$Var1 %!in% Gene_CorrectedSynthetic_lethal$X1 | SL_normal$Var2 %!in% Gene_CorrectedSynthetic_lethal$X2) &
                                    (SL_normal$Var1 %!in% Gene_correctedLethalRxns$Rxns | SL_normal$Var2 %!in% Gene_correctedLethalRxns$Rxns)) 
FP<- 233 

FNlist<-Gene_CorrectedSynthetic_lethal %>% filter(Gene_CorrectedSynthetic_lethal$X1 %!in% SL_normal$Var1 & Gene_CorrectedSynthetic_lethal$X2 %!in% SL_normal$Var2)
FN<-136

TN<-possibleCombinations-28-236-136 #16894
ContMatrix_Normal<- as.data.frame(matrix(c(TP,FN,FP,TN),nrow=2))
ContMatrix_Normal
